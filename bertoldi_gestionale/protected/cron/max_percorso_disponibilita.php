<?php
/* Configurazione log */
$file = '/var/log/csreisen_max_percorso_disponibilita.log';
$time_start = microtime(true);

/* LOG */
file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] START'.PHP_EOL, FILE_APPEND | LOCK_EX);
/* LOG */
...
/* LOG */
file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] include e connessione db eseguita'.PHP_EOL, FILE_APPEND | LOCK_EX);
/* LOG */
...
/* LOG */
file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] eseguita query per recupero corse'.PHP_EOL, FILE_APPEND | LOCK_EX);
/* LOG */

/* LOG */
file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] inizio for each corse'.PHP_EOL, FILE_APPEND | LOCK_EX);
/* LOG */
...
foreach ($corse as $corsa){
	...
	/* LOG */
	file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] prima del grafo'.PHP_EOL.' LineaId = '.$corsa['LineaId'].' and CorsaId = '.$corsa['CorsaId'].PHP_EOL, FILE_APPEND | LOCK_EX);
	/* LOG */
	
	$grafo = new DisponibilitaGraph($corsa['LineaId'], $corsa['CorsaId'], $corsa['AppCalendarioData'], $db, 200);
	
	/* LOG */
	file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] dopo il grafo'.PHP_EOL, FILE_APPEND | LOCK_EX);
	/* LOG */
...
// 				}
            ...
            $sql = "INSERT INTO RT_MaxDisponibilitaPostiCron (LineaId, CorsaId, DataPartenza, Comune, Posti) VALUES
					(".$corsa['LineaId'].",".$corsa['CorsaId'].", '".$corsa['AppCalendarioData']."',".$k.",
							".$tot1." )";
            $db->query($sql);
            
            /* LOG */
            file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] fine insert'.PHP_EOL, FILE_APPEND | LOCK_EX);
            /* LOG */
        }
        unset($grafo);    
}
...
/* LOG */
file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] fine for each corse'.PHP_EOL, FILE_APPEND | LOCK_EX);
/* LOG */

$db->close();

/* LOG */
file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] chiusura database'.PHP_EOL, FILE_APPEND | LOCK_EX);
/* LOG */

/* LOG */
file_put_contents($file, '[' . date('d/m/Y H:i:s') . '] END - Tempo esecuzione: ' . microtime(true) - $time_start .PHP_EOL, FILE_APPEND | LOCK_EX);
/* LOG */
