...
class Autista {
	...
	public function checkPin($pin, $username){
		global $user;
		$db=$this->conn;
		
		$sql = "select *  from RT_ClientApp c
		left join RT_Autisti a on c.AutistaId = a.AutistiId
		where c.Pin = '$pin' and c.Stato = 1 and c.Cancella = 0 and a.Username = '$username'";
		$rows = $db->fetch_array($sql);
		if(count($rows)>0){
			return true;
		}else{
	...
	public function getAllOperatoriForSelect()
	{
		global $user;
		$db=$this->conn;
		$sql = "SELECT OperatoreId, Cognome, Nome, Username From Operatore WHERE Stato=1 AND Cancella=0 ORDER BY Cognome, Nome";
		$rows = $db->fetch_array($sql);
		$arr_flotta = array();
		foreach ($rows as $key=>$row){
			$arr_flotta[$key]['OperatoreId'] = $row['OperatoreId'];
			$arr_flotta[$key]['Operatore'] = $row['Cognome']." ".$row['Nome']." (".$row['Username'].")";
		}
		return $arr_flotta;
	}
