...
class Form
{
    ...
    public function create_select_no_default($label, $name, $id, $class, $ArrObject, $ValueToCheck, $field1, $field2, $additionalParams, $required = 0, $commento = null)
    {
        global $dizionario; ?>
        <div class="<?= $class ?>">
            <label for="<?= $name ?>">
                <?php if ($required)
                    echo ("<span class=\"required\"><span class=\"hidden\">*</span>" . $label . " </span>");
                else
                    echo ($label);
                ?>
            </label>
            <?php if ($class == "brain_campoForm") print("<br />"); ?>
            <select name="<?= $name ?>" id="<?= $id ?>" <?php $this->add_paramms($additionalParams); ?>>
            ...
            <div class="radio_group">
                ...
                while ($i < $ArrObjectSize) {
                    ...
                    if ($value == $ValueToCheck)
                        $check = "checked";
                ?>
                    <input type="radio" name="<?= $name ?>" id="<?= $name . "_" . $value ?>" value="<?= $value ?>" <?= $check ?> <?php $this->add_paramms($additionalParams); ?> />
    ...
    public function create_select($label, $name, $id, $class, $ArrObject, $ValueToCheck, $field1, $field2, $additionalParams, $required = 0, $commento = null)
    {
        ...
        <div class="<?= $class ?>">
            ...
            <?php if ($class == "brain_campoForm") print("<br />"); ?>
            <select name="<?= $name ?>" id="<?= $id ?>" <?php $this->add_paramms($additionalParams); ?>>
                <option value="">- <?= $dizionario['generale']['seleziona'] ?> -</option>
                <?php
                if ($ArrObject) {
                    $ArrObjectSize = count($ArrObject);
                    $i = 0;
    ...
    ?>
        <div class="<?= $class ?>">
            ...
            <?php echo ($br); ?>
            <input size="<?= $inputsize ?>" maxlength="<?= $maxlength; ?>" type="text" name="<?= $name ?>" id="<?= $id ?>" value="<?= $value ?>" <?php $this->add_paramms($additionalParams); ?> />&nbsp;
            <?= $commento ?>
        </div>
    <?
    }
    ...
    ?>
        <div class="<?= $class ?>">
            ...
            <?php if ($class == "brain_campoForm") print("<br />"); ?>
            <input type="file" name="<?= $name ?>" id="<?= $id ?>" value="<?= $value ?>" <?php $this->add_paramms($additionalParams); ?> />
        </div>
    <?
    }
    ...
    ?>
        <div class="<?= $class ?>">
            ...
            <?php print($br); ?>
            <input type="checkbox" name="<?= $name ?>" id="<?= $id ?>" value="<?= $value ?>" <?php $this->add_paramms($additionalParams); ?> />
        </div>
    <?php }
    ...
    ?>
        <div class="<?= $class ?> brainConSelector">
            ...
            <input size="<?= $inputsize ?>" type="text" name="<?= $name ?>" id="<?= $id ?>" value="<?= $value ?>" <?php $this->add_paramms($additionalParams); ?> />
        </div>
    <?php }
    ...
    ?>
        <div class="<?= $class ?>">
            ...
            <?php print($br); ?>
            <input type="password" name="<?= $name ?>" id="<?= $id ?>" value="<?= $value ?>" <?php $this->add_paramms($additionalParams); ?> />
        </div>
    <?php
    }
    ...
    ?>
        <div class="<?= $class ?>">
            ...
            <?php print($br); ?>
            <textarea name="<?= $name ?>" id="<?= $id ?>" <?php $this->add_paramms($additionalParams); ?>><?= $value ?></textarea>
        </div>
    <?php
    }
    ...
    public function create_button($label, $name, $value, $class, $type, $onclick = null)
    {
    ?>
        <input type="<?= $type ?>" name="<?= $name ?>" class="<?= $class ?>" id="<?= $name ?>" value="<?= $value ?>" <?= isset( $onclick ) ? "onclick ='$onclick'" : '' ?>/>
<?php
    }
}
?>