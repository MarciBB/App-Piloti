...
$ModuloId=1;
$aColumns = array('Nazione','Regione','Provincia','Comune','ComuneId','NazioneId');
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == 'Comune' )
			{
				$nome_comune = str_replace("'","\'",$aRow['Comune']);
                             ...
                             $row[] = "<a class=\"seleziona_item\" href=\"javascript:void(0);\" onclick=\"javascript:UpdateComuneFromBox('".$aRow['ComuneId']."','".$nome_comune."');\" >".($aRow[ $aColumns[$i] ])."</a>";
