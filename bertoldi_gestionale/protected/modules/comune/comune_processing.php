...
$ModuloId=1;
$aColumns = array('Comune','Provincia','Regione','Nazione','ComuneId');
$sIndexColumn = "ComuneId";
$sTable = "NazioneRegioneComune";
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == 'Comune' )
			{
			    //$nome_comune = str_replace("'","\'",$aRow['Comune']);
                       ...
                       elseif ( $aColumns[$i] == 'ComuneId' )
			{
				/* General output */
				$ComuneId=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('comune','comune.php?do=edit&amp;ComuneId=".$ComuneId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";                                
