...
$ModuloId=1;
$aColumns = array('Stato','TrattaNome','AppTrattaTipo','AppMezzo','LineaArea','LineaNome','PercorsoNome','TrattaId', 'TrattaId as elimina' );
$sIndexColumn = "TrattaId";
$sTable = "RT_ElencoTratta";
...
$errors = new Errors();
	...
	*/
	...
	if ($sWhere=="")
		$sWhere =" where LineaId <> 14 and OdcIdRef=$OdcIdRef";
	else
		$sWhere.=" and LineaId <> 14 and OdcIdRef=$OdcIdRef";
       
	$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", implode(", ", $aColumns))."
		FROM   $sTable
		$sWhere AND Cancella = 0 ";
	...
	while ( $aRow = $db->fetch_array( $rResult ) ) {
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			} elseif ( $aColumns[$i] == "TrattaId") {
                $TrattaId=$aRow[ $aColumns[$i] ];
				/* General output */
                $row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_tratta','tratta_wz.php?do=edit&amp;TrattaId=".$TrattaId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			} elseif ( $aColumns[$i] == "TrattaId as elimina") {
				/* General output */
				$row[] = "<a title='elimina' href='#' onclick='javascript:CancellaTratta(".$TrattaId.");' title='". $dizionario['autista']['elimina']."'>
