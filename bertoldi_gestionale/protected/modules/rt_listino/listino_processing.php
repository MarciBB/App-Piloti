...
$ModuloId=1;
$aColumns = array('Stato','ListinoNome','ListinoPeso','ListinoId' );
$sIndexColumn = "ListinoId";
$sTable = "RT_ElencoListino";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			elseif ( $aColumns[$i] == "ListinoId")
			{
                            $ListinoId=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_listino','listino.php?do=edit&amp;ListinoId=".$ListinoId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			}
