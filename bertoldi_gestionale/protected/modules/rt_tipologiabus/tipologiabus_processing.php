...
$ModuloId=1;
$aColumns = array('Stato','TipologiaBus','NumeroPiani','TotalePosti','TipologiaBusId' );
$sIndexColumn = "TipologiaBusId";
$sTable = "RT_ElencoTipologiaBus";
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			elseif ( $aColumns[$i] == "TipologiaBusId")
			{
				...
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_tipologiabus','tipologiabus.php?do=edit&amp;TipologiaBusId=".$TipologiaBusId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			}
