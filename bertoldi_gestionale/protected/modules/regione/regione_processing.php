...
$ModuloId=1;
$aColumns = array('Regione','Nazione','RegioneId');
$sIndexColumn = "RegioneId";
$sTable = "RT_ElencoRegione";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == 'Regione' )
			{
			    //$nome_comune = str_replace("'","\'",$aRow['Regione']);
                            $nome_comune = $aRow['Regione'];
                             $row[] = "<strong>".$nome_comune."</strong>";
                            
				
			}
                        
                        elseif (( $aColumns[$i] == 'RegioneId' ))
			{
				/* General output */
                        	  $RegioneId=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('regione','regione.php?do=edit&amp;RegioneId=".$RegioneId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
                               // $row[] = "<a href=\"#\" onclick=\"loadMainContent('regione','regione.php?do=traduci&amp;RegioneId=".$RegioneId."',this);\" title=\"traduci\"><img alt=\"traduci\" title=\"traduci\" src=/images/traduci_item.png /></a>";
			
                    
			}
                        
                      
                   	elseif (( $aColumns[$i] != 'RegioneId' ) and ( $aColumns[$i] != 'NazioneId' ))
			{
				/* General output */
