	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			{
				$nome_comune = htmlentities($aRow['Regione']);
                                $row[] = "<a class=\"seleziona_item\" href=\"javascript:void(0);\" onclick=\"javascript:UpdateComuneFromBox('".$aRow['RegioneId']."','".$nome_comune."');\" >".$nome_comune."</a>";
                        }
