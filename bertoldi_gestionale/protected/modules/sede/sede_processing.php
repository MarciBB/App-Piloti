...
$ModuloId=1;
$aColumns = array('RagioneSociale', 'Comune', 'Indirizzo', 'Telefono', 'Fax','Email','CodiceSede','SedeId');
$sIndexColumn = "SedeId";
$sTable = "ElencoSediView";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == "SedeId")
			{
                            $SedeIdPass=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"javascript:void(0);\" onclick=\"loadMainContent('sede','sede.php?do=edit&amp;SedeId=".$SedeIdPass."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
