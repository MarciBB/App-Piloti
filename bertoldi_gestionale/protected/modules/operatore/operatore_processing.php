...
$ModuloId=1;
$aColumns = array('Stato','Cognome', 'Nome', 'Username','OperatoreTipo','Email','RagioneSociale','OperatoreId');
$sIndexColumn = "OperatoreId";
$sTable = "ElencoOperatoreView";
...
$InGestoreFigli=implode(",", $gestorefigli);
	...
	*/
	...
	if ( isset( $_GET['iSortCol_0'] ) )
	{
		...
		$sOrder = 'ORDER BY  Cognome asc, Nome asc';
	}
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == "OperatoreId")
			{
                            $OperatoreIdPass=$aRow[ $aColumns[$i] ];
				/* General output */
                            $azione="<a href=\"javascript:void(0);\" onclick=\"loadMainContent('operatore','operatore.php?do=edit&amp;operatoreId=".$OperatoreIdPass."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
                            //$azione.="<a href=\"javascript:void(0);\" onclick=\"javascript:eliminaOperatore('action=elimina_operatore&amp;id=".$OperatoreIdPass."');\" title=\"elimina operatore\"><img alt=\"elimina operatore\" title=\"elimina operatore\" src=/images/iconset/cancella_item.png /></a>";
			...
			elseif (( $aColumns[$i] != '' )&&( $aColumns[$i] != 'OperatoreId'))
			{
				/* General output */
				$row[] = $aRow[ $aColumns[$i] ];
			}
