...
<script type="text/javascript" charset="utf-8">                    
    ...
    oTable = $("#brain_datatables").dataTable({
        "bProcessing": true,
        "bServerSide": true,
        "sAjaxSource": "/protected/modules/rt_coupon/coupon_processing.php",
        "aoColumns": [
        ...
        "bAutoWidth": false,
        "oLanguage": {
            "sProcessing": "<?=$dizionario['generale']['caricamento_in_corso']?>",
        ...
        "fnInitComplete": function () {
            // Esempio di creazione di select dinamiche nei filtri (se usi <span> nei th)
            ...
            // Esempio: aggiunta di un bottone custom dopo l'inizializzazione (commentato)
            // if($("#lista_aggiungi").val()>0){
            //     var bottone_aggiungi='<div class="brain_servicebar"><a class="brain_aggiungi" href="javascript:void(0);" onclick="loadMainContent(\'operatore\',\'operatore.php?do=add\',this);" title="Crea operatore">aggiungi operatore</a></div> ';
            //     $("#brain_datatables_length").after(bottone_aggiungi);
            // }
        }
    });

    // Gestione apertura/chiusura dettagli riga (collapse)
    ...
    // Gestione filtri: input e select nella riga dei filtri
    $("#brain_datatables thead tr.brain_tabellaFilter th").each(function (i) {
        // Esegui ricerca su change (select e input)
        $('input, select', this).change(function () {
            if (typeof oTable !== "undefined") {
                oTable.fnFilter(this.value, i);
            }
        });
        // Esegui ricerca anche su invio (input text)
