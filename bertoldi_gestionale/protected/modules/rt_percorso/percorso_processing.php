...
$ModuloId=1;
$aColumns = array('Stato','PercorsoNome','PercorsoPeso','PercorsoId' );
$sIndexColumn = "PercorsoId";
$sTable = "RT_ElencoPercorso";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) ) {
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			elseif ( $aColumns[$i] == "PercorsoId")
			{
                            $PercorsoId=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_percorso','percorso.php?do=edit&amp;PercorsoId=".$PercorsoId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			}
                   	...
                   	elseif ( $aColumns[$i] != '' )
			{
                ...
                //$row[] = "<a href=\"#\" onclick=\"javascript:ResetPercorsoBreve();\" title=\"reset\"><i class=\"fa fa-refresh green\" aria-hidden=\"true\" alt=\"blocca\" title=\"blocca - sblocca\"></i></a>";
