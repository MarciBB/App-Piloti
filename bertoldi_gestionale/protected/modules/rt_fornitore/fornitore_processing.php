...
$ModuloId=1;
$aColumns = array('Stato', 'Nome', 'Categoria', 'FornitoreId');
$sIndexColumn = "FornitoreId";
$sTable = "RT_Fornitori";
	...
	while ( $aRow = $db->fetch_array( $rResult ) ) {
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ) {
			...
			if ( $aColumns[$i] == "Stato") {
				...
				/* General output */
			} elseif ( $aColumns[$i] == "Categoria") {
				$CategoriaId = $aRow[ $aColumns[$i] ];
				$categoria = $fornitoreObj->getCategorieById($CategoriaId);
				$row[] = $categoria['Nome'];
			} elseif ( $aColumns[$i] == "FornitoreId") {
				
				$FornitoreId = $aRow[ $aColumns[$i] ];
				/* General output */
				$row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_fornitore','fornitore.php?do=edit&amp;FornitoreId=".$FornitoreId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			} elseif ( $aColumns[$i] != '' ) {
				/* General output */
				$row[] = $aRow[ $aColumns[$i] ];
			}
