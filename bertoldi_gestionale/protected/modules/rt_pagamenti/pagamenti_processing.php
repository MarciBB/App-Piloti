...
$ModuloId=1;
$aColumns = array('Stato', 'PagamentoTipo', 'EtichettaRimborso', 'SupplementoFisso', 'SupplementoPercentuale', 'TempoScadenza', 'MsgSMSStato', 'MsgEmailStato', 'GestorePrimario', 'VisualizzaInRimborso', 'PagamentoTipoId');
$sIndexColumn = "PagamentoTipoId";
$sTable = "RT_PagamentoTipo";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) ) {
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ) {
			...
			} elseif ( $aColumns[$i] == "PagamentoTipoId") {
				
				$PagamentoTipoId=$aRow[ $aColumns[$i] ];
				/* General output */
				$row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_pagamenti','pagamenti.php?do=edit&amp;PagamentoTipoId=".$PagamentoTipoId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
				
			} elseif($aColumns[$i] == "MsgSMSStato") {
			...
			} elseif($aColumns[$i] == "MsgEmailStato") {
			...
			} elseif($aColumns[$i] == "GestorePrimario") {
			...
			} elseif($aColumns[$i] == "VisualizzaInRimborso") {
