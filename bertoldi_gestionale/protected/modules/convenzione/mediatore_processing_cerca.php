...
$ModuloId=1;
$aColumns = array('MediatoreTipo','CognomeRagioneSociale', 'Nome', 'CodiceFiscale', 'PartitaIva','IndirizzoResidenza','ComuneResidenza','MediatoreId');
$sIndexColumn = "MediatoreId";
$sTable = "MediatoreListaFiltroMaterie";
...
global $mediazione_wizard;
$mediazione_wizard=null;

if(isset($_SESSION['MEDIAZIONE_WIZARD'])) {
$mediazione_wizard=unserialize($_SESSION['MEDIAZIONE_WIZARD']);
}

$MediazioneMateriaId=$mediazione_wizard->MediazioneDatiGenerali['MateriaId'];
	...
	*/
	 ...
	 */
        ...
        if ($sWhere=="")
            $sWhere =" where OdcIdRef=$OdcIdRef and MateriaId=$MediazioneMateriaId";
        else
            $sWhere.=" and OdcIdRef=$OdcIdRef and MateriaId=$MediazioneMateriaId";
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == "CognomeRagioneSociale" )
			{
				
					$nome_mediatore = str_replace("'","\'",$aRow[ $aColumns[$i]]);
                                    $row[] = "<a class=\"seleziona_item\" href=\"javascript:void(0);\" onclick=\"javascript:UpdateFieldFromBox('".$aRow['MediatoreId']."','".$nome_mediatore."');\" >".ucfirst($aRow[ $aColumns[$i] ])."</a>";
                             
			}
                      
                    
                   	elseif ( $aColumns[$i] != 'MediatoreId' )
			{
				/* General output */
				$row[] = $aRow[ $aColumns[$i] ];
			}
