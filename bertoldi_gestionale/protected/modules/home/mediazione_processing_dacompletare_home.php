...
$dataoracorrente=date("Y-m-d");
	...
	*/
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
       ...
       if ($sWhere=="")
            $sWhere =" where GestoreId IN ($InGestoreFigli) and OperatoreId=$user->OperatoreId and DATE_FORMAT(DataIns, '%Y-%m-%d')='$dataoracorrente'";
        else
            $sWhere.=" and GestoreId IN ($InGestoreFigli) and OperatoreId=$user->OperatoreId and DATE_FORMAT(DataIns, '%Y-%m-%d')='$dataoracorrente'";
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
                        ...
                        elseif ( $aColumns[$i] == "Mediazioneid")
			{
				...
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('mediazione','mediazione.php?do=edit&amp;MediazioneId=".$MediazioneId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
...
// $row[] = "<img src=/DataTables/examples/examples_support/details_open.png />";
                         ...
                         elseif ( $aColumns[$i] == "Indirizzo")
			{
                             ...
                             $row[] = $aRow[ $aColumns[$i] ]." ".$aRow['Comune'];
			}
                       
                        
			else if ( $aColumns[$i] != 'Comune' )
			{
				/* General output */
				$row[] = $aRow[ $aColumns[$i] ];
			}
