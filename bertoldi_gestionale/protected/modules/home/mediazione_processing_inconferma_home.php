...
$ModuloId=5;
$aColumns = array('InAttesaDiDati','Codice', 'Oggetto', 'Materia', 'data','CodiceSede','Mediazioneid','InAttesaDiDatiId');
...
$InGestoreFigli=implode(",", $gestorefigli);
	...
	*/
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
       ...
       if ($sWhere=="")
            $sWhere =" where GestoreId IN ($InGestoreFigli) and MediazioneStatoId=2 ";
        else
            $sWhere.=" and GestoreId IN ($InGestoreFigli) and MediazioneStatoId=2 ";
...
// $row[] = "<img src=/DataTables/examples/examples_support/details_open.png />";
			...
			else if (( $aColumns[$i] != 'Comune' ) and ( $aColumns[$i] != 'Indirizzo' ) and ( $aColumns[$i] != 'InAttesaDiDatiId' ))
			{
				/* General output */
