...
$ModuloId=3;
$aColumns = array('PromoterCode','CognomeRagioneSociale', 'Nome', 'CodiceFiscale', 'PartitaIva','IndirizzoResidenza','Comune','sigla','PromoterId');
$sIndexColumn = "PromoterId";
$sTable = "PromoterLista";
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
                    ...
                    if ( $aColumns[$i] == "CognomeRagioneSociale" )
			{
					...
					$nome_mediatore = str_replace("'","\'",$aRow[ $aColumns[$i]]);
                                        if ($aRow['Nome'])
                                        $nome_mediatore.=" ".str_replace("'","\'",$aRow['Nome']);
                                           $row[] = "<a class=\"seleziona_item\" href=\"javascript:void(0);\" onclick=\"javascript:UpdateFieldFromBox('".$aRow['PromoterId']."','".$nome_mediatore."');\" >".($aRow[ $aColumns[$i] ])."</a>";
                             
			}
                        elseif ( $aColumns[$i] != "PromoterId")
			{
                           $row[] = $aRow[ $aColumns[$i] ];
			}
