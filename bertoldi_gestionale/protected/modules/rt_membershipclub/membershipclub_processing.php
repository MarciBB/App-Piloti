...
$ModuloId=32;
$aColumns = array('Stato','MembershipClubCode','CognomeRagioneSociale', 'Nome', 'CodiceFiscale', 'PartitaIva','Email','Telefono','MembershipClubId');
$sIndexColumn = "MembershipClubId";
$sTable = "RT_MembershipClub";
...
global $dizionario;
	...
	*/
	 ...
	 */
        ...
        if ($sWhere=="")
            $sWhere =" where OdcIdRef=$user->OdcId ";
        else
            $sWhere.=" and OdcIdRef=$user->OdcId ";
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == "CognomeRagioneSociale" )
			{
				...
				$row[] = "<strong>".htmlentities($aRow[ $aColumns[$i] ])."</strong>";
			}
                         elseif ( $aColumns[$i] == "Stato")
			{
				...
				/* General output */                                
                        ...
                        elseif ( $aColumns[$i] == "MembershipClubId")
			{
                            $MembershipClubId=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_membershipclub','membershipclub.php?do=edit&amp;MembershipClubId=".$MembershipClubId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			}
