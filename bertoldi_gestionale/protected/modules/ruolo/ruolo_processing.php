...
$ModuloId=1;
$aColumns = array('Stato','Nome','Odc','RuoloId');
$sIndexColumn = "RuoloId";
$sTable = "RuoloView";
...
global $dizionario;
	...
	*/
	...
	$sQuery = "
		...
		FROM   $sTable
		WHERE GestoreIdRef IN ($InGestoreFigli)
	";
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == "RuoloId")
			{
                            $RuoloIdPass=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"javascript:void(0);\" onclick=\"loadMainContent('ruolo','ruolo.php?do=edit&amp;ruoloId=".$RuoloIdPass."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			...
			elseif (( $aColumns[$i] != '' )and($aColumns[$i] != 'Stato')and($aColumns[$i] != 'RuoloId'))
			{
				/* General output */
				$row[] = $aRow[ $aColumns[$i] ];
			}
