...
$ModuloId=1;
$aColumns = array('AnagraficaTipoSigla', 'CognomeRagioneSociale', 'Nome', 'CodiceFiscale', 'PartitaIva','IndirizzoResidenza','ComuneResidenza','AnagraficaId');
$sIndexColumn = "AnagraficaId";
$sTable = "AnagraficaLista";
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == "CognomeRagioneSociale" )
			{
                                 ...
                                 $row[] = "<a class=\"seleziona_item\" href=\"javascript:void(0);\" onclick=\"javascript:AddAnagraficaParteIntoMediazione('".$aRow['AnagraficaId']."');\" >".$aRow[ $aColumns[$i] ]."</a>";
			}
                        elseif ( $aColumns[$i] == "AnagraficaTipoSigla")
			{
                             $tipo=$aRow[ $aColumns[$i] ];
                               $row[] = "<span class=brain_tiposiglalista".$tipo.">".$tipo."</span>";
			}
                    
                   	elseif ( $aColumns[$i] != 'AnagraficaId' )
			{
				/* General output */
				$row[] = $aRow[ $aColumns[$i] ];
			}
