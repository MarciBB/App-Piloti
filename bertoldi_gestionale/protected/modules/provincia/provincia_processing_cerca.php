...
$sTable = "ViewProvinciaRegioneNazione";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == 'Provincia' )
			{
				$nome_comune = str_replace("'","\'",$aRow['Provincia']);
                             $row[] = "<a class=\"seleziona_item\" href=\"javascript:void(0);\" onclick=\"javascript:UpdateComuneFromBox('".$aRow['ProvinciaId']."','".$nome_comune."');\" >".ucfirst($aRow[ $aColumns[$i] ])."</a>";
                        }
