...
$ModuloId=1;
$aColumns = array('Stato','NomeRegola','GiorniPrima','OrePrima','TipoPrenotazione', 'TipoModifica' );
$sIndexColumn = "ModificaRegolaId";
$sTable = "RT_ModificaRegola";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			{
                            $RegolaRimborsoId=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_regole','regole.php?do=edit&amp;RimborsoRegolaId=".$RegolaRimborsoId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			}
