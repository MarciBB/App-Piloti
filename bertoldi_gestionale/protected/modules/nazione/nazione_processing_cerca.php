	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			{
				$nome_comune = str_replace("'","\'",$aRow['Nazione']);
                             $row[] = "<a class=\"seleziona_item\" href=\"javascript:void(0);\" onclick=\"javascript:UpdateComuneFromBox('".$aRow['NazioneId']."','".$nome_comune."');\" >".ucfirst($aRow[ $aColumns[$i] ])."</a>";
