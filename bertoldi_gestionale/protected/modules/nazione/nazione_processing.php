...
$ModuloId=1;
$aColumns = array('Nazione','NazioneId');
$sIndexColumn = "NazioneId";
$sTable = "Nazione";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == 'Nazione' )
			{
			    $nome_comune = $aRow['Nazione'];
                             $row[] = "<strong>".$nome_comune."</strong>";
                            
				
			}
                        
                        elseif (( $aColumns[$i] == 'NazioneId' ))
			{
				/* General output */
                        	  $NazioneId=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('nazione','nazione.php?do=edit&amp;NazioneId=".$NazioneId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
               //                 $row[] = "<a href=\"#\" onclick=\"loadMainContent('nazione','nazione.php?do=traduci&amp;NazioneId=".$NazioneId."',this);\" title=\"traduci\"><img alt=\"traduci\" title=\"traduci\" src=/images/traduci_item.png /></a>";
			}
