...
$ModuloId=1;
$aColumns = array('Stato','Modello','Targa','TipologiaBus','NumeroPiani','TotalePosti','FlottaId');
$sIndexColumn = "FlottaId";
$sTable = "RT_ElencoFlotta";
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			elseif ( $aColumns[$i] == "FlottaId")
			{
				...
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_flotta','flotta.php?do=edit&amp;FlottaId=".$FlottaId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			}
