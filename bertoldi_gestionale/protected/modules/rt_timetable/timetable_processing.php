...
$ModuloId=1;
$aColumns = array('Stato','LineaNome','LineaArea','PercorsoNome','LineaId' );
$sIndexColumn = "LineaId";
$sTable = "RT_ElencoLinea";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) ) {
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			{
                            $LineaId=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"#\" onclick=\"loadMainContent('rt_timetable','timetable.php?do=edit&amp;LineaId=".$LineaId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
			}
