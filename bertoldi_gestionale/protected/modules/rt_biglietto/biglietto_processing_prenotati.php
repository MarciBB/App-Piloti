    ...
    if (isset($_GET['bSearchable_' . $i]) && $_GET['bSearchable_' . $i] == "true" && $_GET['sSearch_' . $i] != '') {
        ...
        if ($aColumns[$i] == 'NumeroBiglietto') {
            $temp = "(case
                ...
                then concat(convert( `rdc`.`Codice` using utf8),
                        _utf8'/',
        ...
        $sWhere .= " AND " . $temp . " LIKE '%" . $db->escape($_GET['sSearch_' . $i]) . "%' ";
    }
}
...
if ($sOrder == "") {
    if ($Order_post == '1') {
        $sOrder = "order by pd.DataArrivo asc, pd.OrarioArrivo asc, pd.ComuneArrivo";
    } else {
