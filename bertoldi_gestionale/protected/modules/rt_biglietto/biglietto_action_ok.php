...
function RimborsoParziale()
{
    ...
    $modifica = 0;
    echo ("ok" . '_' . $modifica . '_' . $PrenotazioneId . '_' . 3);
    die();
}
...
function GetImportoMassimoRimborsabile() {
	...
	$sql = "SELECT SUM(Importo) Importo FROM RT_PrenotazioneDettaglio WHERE PrenotazioneNumero = $PrenotazioneNumeroId";
	if ($Tragitto != '0') {
		$sql .= " AND Tragitto = '$Tragitto'";
	}
...
function GetTipologiaBiglietti() {
	...
	$arrPrezziBiglietti = $prenotazione->GetTipologiaBigliettiPrezzi(0, $PrenotazioneId, $prenotazione_wizard, $Data, $CorsaId, $FermataIdAP, $FermataIdAD, $CorsaRitornoId, $FermataIdRP, $FermataIdRD, $TV, null, null, null );
	/*
        	...
        	<tr class="rowBianca">
				...
				<th scope="row">
               	...
               	<?
					...
					/*
					 * $page->create_textbox($BigliettoPrezzo,"Pax".$i,"BigliettoTipologiaPax[$stringa]",0,1,"brain_campiform",array( "onChange"=>"'javascript:CalcolaPrezzoTipoBiglietto($BigliettoId,\"$BigliettoPrezzo\",$PrezzoPax,this);'", "class"=>"'required number'"));
					 */
				...
				<td><input <?=$readonly?> id="PrezzoRiduzione<?=$x?>"
...
$sql = "SELECT distinct FermataId,FermataOrario,TrattaNome From RT_ElencoFermataOrarioPK WHERE IsPickup=1 and  CorsaId=$CorsaId and ComuneId=$ComuneAndataId  and OdcIdRef=$user->OdcId and TrattaId=$trattaPartenza order by TrattaPeso desc ";      

$arr_fermate=$db->fetch_array($sql);

$sql = "SELECT distinct FermataIdDrop,FermataOrarioDrop,TrattaNome From RT_ElencoFermataOrarioDO WHERE IsDropOff=1 and  CorsaId=$CorsaId and ComuneId=$ComuneRitornoId  and TrattaId=$trattaArrivo and OdcIdRef=$user->OdcId order by TrattaPeso asc";      
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" id="gestoreElencoAule">
                                ...
                                <tr class="rowBianca">
                                    ...
                                    <td><span>
                                    attiva                                    </span></td>
                                         <td><a title="edita" onclick="javascript:ExternalLoad('rt_fermata','fermata.php?do=edit&amp;FermataId=9');" href="#"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a></td>
...
function create($modifica = 0) {
	...
	if (! empty ( $row ['DataIns'] )) {
		...
		if ($secondi <= 10)
			$err .= "Il sistema ha tentato di inserire due volte la prenotazione. E' stata memorizzata una sola prenotazione";
		// echo($secondi);
	}

	if ($CorsaRitorno > 0) {
		$sqA = "select PercorsoId from RT_ElencoCorsa where CorsaId=$CorsaAndata";
		$rowA = $db->query_first ( $sqA );
		$PercorsoAndataId = $rowA ['PercorsoId'];

		$sqR = "select PercorsoId from RT_ElencoCorsa where CorsaId=$CorsaRitorno";
	...
	if ($check == true) {
		...
		if ($lastidA != false) {
			...
			$arrPrezzi = $prenotazione->GetTipologiaBigliettiPrezzi ( 1, $NuovaPrenotazioneId, null, $DataSelezionataA, $CorsaAndata, $FermataAndataP, $FermataAndataD, $CorsaRitorno, $FermataRitornoP, $FermataRitornoD, $TV, $arr_biglietti_prenotati, $arr_biglietti_aumento, $arr_biglietti_riduzione );
...
function ConfermaPrenotazioneLista() {
	...
	$sql = "select * from RT_CheckPrenotazioneDaConfermare where OdcIdRef=$user->OdcId and PrenotazioneId=$PrenotazioneId";
	$row = $db->query_first ( $sql );
