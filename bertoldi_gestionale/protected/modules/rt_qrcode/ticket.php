...
require_once($basepath."/protected/modules/rt_previaggio/stampa_titoli_di_viaggio_func.php");
...
if (count($prenotazioni) > 0) {
	$titolo = $db->query_first("SELECT PrenotazioneTitoloId, TipoTitolo FROM RT_PrenotazioneTitolo WHERE PrenotazioneNumeroId = ".$PrenotazioneNumeroId);
	$PrenotazioneTitoloId = isset($titolo['PrenotazioneTitoloId']) ? $titolo['PrenotazioneTitoloId'] : 0;
	$TipoTitolo = isset($titolo['TipoTitolo']) && $titolo['TipoTitolo'] != '' ? $titolo['TipoTitolo'] : 'E';
	stampa_titoli_di_viaggio(
			$prenotazioni[0]['PrenotazioneId'],
			$prenotazioni[0]['DataInizioItinerario'],
			$prenotazioni[0]['CorsaId'],
			$prenotazioni[0]['Tragitto'],
			$PrenotazioneTitoloId,
			$TipoTitolo,
			true,
			null,
			true
...
function stampa_titoli_di_viaggio_old($CorsaId, $DataPartenza, $Tragitto, $PrenotazioneId, $PrenotazioneNumeroId) {
	global $dizionario;
	...
	$pdf->setSourceFile($basepath.'/protected/modules/rt_previaggio/pdftemplate/voucherweb.pdf');
	...
	global $abilita_modifica,$tratta_wizard,$db,$user;
 	...
 	$titoloTemp = $db->query_first($sql);
	// serve per definire il tipo di stampa
	$PrenotazioneTitoloId = $titoloTemp['PrenotazioneTitoloId'];
	$tipo_titolo="E";
	...
	//$gestore=new Gestore();
	//$gestore->conn=$db;
	//$gestorefigli=$gestore->getGestoreFigli($user->GestoreId);echo "ciao";die();
	...
	$sql="select * from RT_PrenotazioneDettaglioCompleto where Stato=1 and Cancella=0 and PrenotazioneTitoloId is not null and PrenotazioneId=$PrenotazioneId and TipoServizio='Bus' and Tragitto='$Tragitto' and $sq_tipo_titolo and CorsaId=$CorsaId and DataInizioItinerario='$DataPartenza' order by ClienteNome asc";
	...
	while ($np<$numeropasseggeri)
	{
		...
		// rimborso
			...
			if(isset($rowCoupon['CouponId'])){
				...
				$pdf->SetXY($base2X - 8, $base2Y + 115);
				$pdf->Write(0, "Il Codice Coupon �: ".$rowCoupon['Codice']);
				$pdf->SetXY($base2X - 8, $base2Y + 120);
		...
		if (Config::$covidMode == true){
			//$docAdd = $basepath.'/protected/modules/rt_previaggio/pdftemplate/Autodichiarazione_ingresso.pdf';;
			...
			$docAdd = '/protected/modules/rt_previaggio/pdftemplate/informazioni_covid_25_01_2022.pdf';
			$pagecount = $pdf->setSourceFile($docAdd);
			for($i=0; $i<$pagecount; $i++){
				$pdf->AddPage();
				$tplidx1 = $pdf->importPage($i+1, '/MediaBox');
...
// 			$docAdd = '/protected/modules/rt_previaggio/pdftemplate/modulo_rientro_sintetico_05_gennaio_2021.pdf';
// 			$pagecount = $pdf->setSourceFile($docAdd);
// 			for($i=0; $i<$pagecount; $i++){
// 				$pdf->AddPage();
// 				$tplidx1 = $pdf->importPage($i+1, '/MediaBox');
