...
$ModuloId=33;
$aColumns = array('Provvigione','DescrizioneProvvigione','ProvvigioneId');
$sIndexColumn = "ProvvigioneId";
$sTable = "ProvvigioneLista";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == "ProvvigioneId")
			{
                            $ProvvigioneId=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"javascript:void(0);\" onclick=\"loadMainContent('provvigione','provvigione.php?do=edit&amp;ProvvigioneId=".$ProvvigioneId."',this);\" title=\"edita\"><i class=\"fa fa-pencil-square-o edita\" aria-hidden=\"true\" alt=\"edita\" title=\"edita\"></i></a>";
