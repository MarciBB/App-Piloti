...
$q="SELECT * from stat_MaturatoNonFatturatoSoggettoParte where  GestoreIdRef IN ($InGestoreFigli) and OdcIdRef=$user->OdcId ";
...
$q.="order by RagioneSociale,SedeIns,Comune,Indirizzo,Mediazioneid asc";



$rep->SetQuery($q);
//$rep->AddGroupingField('DataFattura_formattata','DataFattura_formattata ','DataFattura: ','totale fatturato il %name%');
$rep->AddGroupingField('RagioneSociale','RagioneSociale ','Gestore: ','Maturato per mediazioni fatte da %name%');
$rep->AddGroupingField('Comune','Comune ','Sede: ','Maturato per la sede di %name%');
...
$rep->AddField('MediazioneStato','Stato Mediazione');
$rep->AddField('Codice','Protocollo');
$rep->AddField('Debitore','Debitore');
//$rep->AddField('MediazionePagamentoRif','Riferimento');
$rep->AddField('DataPresentazione','Presentazione Istanza');
$rep->AddField('giorni','Scaduto da giorni');
$rep->AddField('DataPrimoIncontro','Primo Incontro');
$rep->AddField('giorni_incontro','Scaduto da giorni');
$rep->AddField('PatrocinioGratuito1','Gratuito Patrocinio');
//$rep->AddField('Imponibile','Imponibile (&euro;)',1,'','money');
//$rep->AddField('Iva','Iva (%)',1,'','money');
$rep->AddField('Residuo','Da Incassare (&euro;)',1,'','money');
$rep->AddField('Esigibile','Esigibile (&euro;)',1,'','money');
...
$rep->SetSummary('<strong>TOTALE MATURATO NON FATTURATO (&euro;) <strong> ');
...
$titolo_report="Report relativo al Maturato non ancora Fatturato<br />";
