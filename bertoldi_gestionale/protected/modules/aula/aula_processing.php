...
$ModuloId=1;
$aColumns = array('Aula','Capienza','Indirizzo', 'Comune', 'RagioneSociale','AulaId');
$sIndexColumn = "AulaId";
$sTable = "ElencoAuleView";
	...
	*/
	...
	while ( $aRow = $db->fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] == "AulaId")
			{
                            $AulaIdPass=$aRow[ $aColumns[$i] ];
				/* General output */
                                $row[] = "<a href=\"javascript:void(0);\" onclick=\"loadMainContent('aula','aula.php?do=edit&amp;AulaId=".$AulaIdPass."',this);\" title=\"edita\"><img alt=\"edita\" title=\"edita\" src=/images/edita_item.png /></a>";
