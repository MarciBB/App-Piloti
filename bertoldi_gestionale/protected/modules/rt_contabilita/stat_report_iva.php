...
include_once("stat_report_iva_validator.php");  
$datacorrente=Date('d/m/Y');

$sql="select ConfineId,Confine from RT_Confine where Stato=1 and Cancella=0";
$arr_confine = $db->fetch_array($sql);
...
<?
    /*$page->create_select("Agenzia","GestoreId","GestoreId","rowForm",$arr_gestore,$gestorecorrente,"GestoreId","RagioneSociale",
    array("onChange"=>"'javascript:getSediByGestore(this);'"
                                                   ),1);
 
 
    $page->create_select("Rivendita","SedeId","SedeId","rowForm",$arr_sedi,$user->SedeId,"SedeId","Sede",
                                             null,1);*/
    
    
                                 $page->create_select($dizionario['conto']['territorio'],"ConfineId","ConfineId","rowForm",$arr_confine,0,"ConfineId","Confine",
    array(null),1);            
    ?>
						...
						<label for="dataAl"><?=$dizionario['generale']['dal']?></label>
