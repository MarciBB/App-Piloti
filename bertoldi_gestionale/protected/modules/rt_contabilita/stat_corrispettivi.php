...
function show_list() {
    ...
    include_once("stat_corrispettivi_validator.php");  
    ...
    <!--inizio riga per i filtri--->		
    	<div class="brainFiltri">
			...
			<?php
                $page->create_select($dizionario['percorso']['percorso'].":","PercorsoId","PercorsoId","rowForm",$arr_linee,0,"PercorsoId","PercorsoNome", array(null),1);            
            ?>
                                    
            <div class="rowForm">
				<label for="servizio"><?=$dizionario['tipo_big']['servizio']?>:</label>
				<select name="servizio" id="servizio" class="required">
                	<option value="" selected >-- <?=$dizionario['generale']['seleziona']?> --</option>              
                  	<option value="Totali" selected><?=$dizionario['conto']['totali']?></option>
                  	<option value="agenzie" ><?=$dizionario['conto']['gestore_agenzie']?></option>                         
				</select>
		    </div>
