...
$q="SELECT Articolo as Periodo,
        ...
        (CASE
            WHEN Tipo = 'credit_note' THEN '".$dizionario['biglietto']['credit_note']."'
            WHEN Tipo = 'receipt' THEN '".$dizionario['biglietto']['receipt']."'
            ELSE '".$dizionario['biglietto']['invoice']."'
        END) as Tipo
...
if (isset($post_tipo) && $post_tipo != '') {
	$qw.="and Tipo = '$post_tipo' ";
}
...
$rep->AddField('Agenzia',$dizionario['gestore']['ragione_sociale']);
$rep->AddField('Tipo',$dizionario['biglietto']['tipo']);
$rep->AddField('FatturaData',$dizionario['generale']['data']);
$rep->AddField('FatturaNumero', $dizionario['generale']['fattura_numero']);
$rep->AddField('Periodo', $dizionario['generale']['descrizione']);
...
$titolo_report=$dizionario['menu_voci']['51']."<small><a href='#' class='exportToExcel'>".$dizionario['stampe']['esporta']."</a></small><br />";
