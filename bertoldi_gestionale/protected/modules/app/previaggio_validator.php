...
function loadMediazioneStep(modulo,page,elem)
    ...
    function submit_form_previaggio(){
        ...
        var CorsaId='<?=isset($CorsaId)? $CorsaId : ''?>';
        var DataPartenza='<?=isset($DataPartenza)? $CorsaId : ''?>';
   ...
   $(document).ready(function() {
	   ...
	   //verifico la data di oggi con quella selezionata per far comparire il tasto di prentoazione
	   var d = new Date();
	   var from = $('#DataPartenza').val().split("/");
	   var f = new Date(from[2], from[1] - 1, from[0]);
	   var dSelezionato = new Date();
	   ...
	   if(f.toDateString() == d.toDateString()){
    	...
    	$("#application_form_addBus").validate({
         ...
         $('#CorsaId').change(function(){
        	//verifico la data di oggi con quella selezionata per far comparire il tasto di prentoazione
      	   	var d = new Date();
			var from = $('#Dal').val().split("/")
			var f = new Date(from[2], from[1] - 1, from[0])
			var dSelezionato = new Date();
