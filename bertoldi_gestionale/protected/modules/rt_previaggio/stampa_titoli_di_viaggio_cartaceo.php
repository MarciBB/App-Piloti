...
/* funzione per la stampa
 function stampa_titolo_di_viaggio($TitoloId) {
 if (!isset($TitoloId)) {
 echo "Titolo inesistente";
 exit;
 }
 ...
 $sql = "SELECT pt.PrenotazioneTitoloId, pt.PrenotazioneNumeroId, pt.Codice, pt.Anno, pt.Progressivo, pt.TipoTitolo, pt.ImportoTitolo, pt.PostViaggio, pn.TipologiaBigliettoId, pp.Cognome, pp.Nome, pp.SessoId, pp.Eta
 FROM RT_PrenotazioneTitolo pt
 LEFT JOIN RT_PrenotazioneNumero pn ON (pt.PrenotazioneNumeroId = pn.PrenotazioneNumeroId)
 LEFT JOIN RT_PrenotazionePasseggeri pp ON (pn.PasseggeroId = pp.PrenotazionePasseggeroId)
 WHERE pt.PrenotazioneTitoloId=$TitoloId";
    ...
    if (isset($_REQUEST['TipoTitolo']))
        ...
        if ($user->GestoreId==1)
            ...
            while ($np<$numeropasseggeri) {
				...
				$pdf->setSourceFile('pdftemplate/voucherweb_cartaceo.pdf');
...
;
                    ...
                    if ($tipo_titolo=='E' || $tipo_titolo=='X') {
						...
						//se il biglietto � di tipo A/R aggiungo pagina RITORNO
						if($TipoViaggioId == 2) {
