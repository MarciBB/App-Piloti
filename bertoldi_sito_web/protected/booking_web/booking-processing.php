...
$pdftemlatepath=$basepath."/protected/booking_web/pdftemplate/";
$config=new Config();
$run=$config->load();
...
$tmpl_percorso = $pdftemlatepath."/voucher.html";	
...
while($x<sizeof($ArrObject))
{
	...
	$ShuttleRitornoArrivoOra = "#####";
     ...
     $IdPrenotazione=$ArrObject[$x]['PrenotazioneId'];
   ...
   //  die($sql);
	...
	$BusAndataArrivoOra = substr($row['OrarioArrivo'],0,5);
        ...
        $dt=new DT($row['DataIns'],'Y-m-d');
        $DataTitolo=$dt->getDate('d/m/Y');
        
       
             $Euro = chr(128);
	//$Titolo = "Titolo";
        
        $Titolo=EmettiBiglietti($PercorsoId,$PrenotazioneNumero,$IdPrenotazione);
        $DataEmissione = $DataTitolo;
	...
	$pdf->setSourceFile($pdftemlatepath.'/voucherweb.pdf'); 
   ...
   // die();
    $PrenId.=",".$PrenotazioneId;
     //$content="ciao";
     $email_from_address=  Config::$BookingEmail;
     $name_from_address=  Config::$BookingNameEmail;
     $oggettoemail=  Config::$BookingEmailSubject;
     $inviatoda=$$name_from_address. '<'.$email_from_address.'>';
     $emailsubject=$oggettoemail." ".$PrenotazioneNumero."_".$CodicePrenotazione;
...
$dup=null;
$dup['PrenotazioneStato']=3;
$up1=$db->update("RT_Prenotazione", $dup,"PrenotazioneId IN ($PrenId)");
$up2=$db->update("RT_PrenotazionePercorso", $dup,"PrenotazioneId IN ($PrenId)");
$dup1=null;
$dup1['Notificata']=1;
...
$up3=$db->update("RT_PrenotazioneTransazione", $dup1,"PrenotazioneId=".$PrenotazioneId);
...
function EmettiBiglietti($PercorsoId,$PrenotazioneNumeroId,$PrenotazioneId)
{
     ...
     $sql="select SedeIns,OpeIns,GestoreIdRef from RT_PrenotazioneDettaglio where PrenotazioneId=$PrenotazioneId limit 1 ";
     $row1 = $db->query_first($sql);
     
     $sql="Select * from RT_Percorso where PercorsoId=$PercorsoId";
     $row = $db->query_first($sql);
     $Identificativo=$row['Identificativo'];
    
        $anno=Date('Y');
        $progressivo=GetProgressivoTitoloDiViaggio($anno,$PercorsoId); // get codice
        $codice=$Identificativo."-".GetCodiceTitoloDiViaggio($anno,$PercorsoId); // get codice
        $d1['Anno']=$anno;
        $d1['Codice']=$codice;
        ...
        $d1['OpeIns']=$row1['OpeIns'];
        $d1['SedeIns']=$row1['SedeIns'];
        $d1['DataIns']=date('Y-m-d H:i:s');
        $d1['IpIns']=getenv('REMOTE_ADDR');  
        $d1['OdcIdRef']=$OdcId;
        $d1['GestoreIdRef']=$row1['GestoreIdRef'];
        $d1['Cancella']=0;
        $d1['Stato']=1;
...
function GetCodiceTitoloDiViaggio($y,$PercorsoId)
{
global $db;
   

$sql = "SELECT * from RT_CalcolaCodiceBiglietto where Anno=$y and OdcIdRef=1 and PercorsoId=$PercorsoId";
...
function GetProgressivoTitoloDiViaggio($y,$PercorsoId)
{
global $db;
...
$row = $db->query_first($sql);
$ProgressivoOdc=1;
if ( (!empty($row['AttualeNumero'])) and ($row['AttualeNumero']>0))
    $ProgressivoOdc=$row['AttualeNumero']+1;




return $ProgressivoOdc;
