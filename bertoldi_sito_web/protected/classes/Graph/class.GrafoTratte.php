...
class GrafoTratte {
	...
	function __construct($idLinea, $idCorsa, $conn, $partenza, $destinazione, $check = false) {
		...
		if($result['km']>-1){
			...
			if (! isset ( $idCorsa ) || $this->ritornoOpen == 1) {
				...
				$sql = "SELECT DISTINCT t.TrattaId 
						FROM RT_Tratta t 
						LEFT JOIN RT_CorsaTariffa ct ON (t.TrattaId = ct.TrattaId)
						WHERE t.Stato = 1 AND 
						t.Cancella = 0 AND 
				...
				if(count($rowsTratte) == 0){
					...
					$sql = "SELECT DISTINCT t.TrattaId
							...
							t.Cancella = 0 AND
							t.LineaId = $this->idLinea AND
							t.DaConfermare = 0 ORDER BY TrattaPeso DESC";
		...
		foreach ( $this->graph->nodes [$this->graph->edges [$edgeId]->nodeA]->busArrivo as $nomeBus => $bus ) {
			...
			if ($this->graph->edges [$edgeId]->info2 > 0) {
				...
				if ($this->flotta [$bus]->comuni [$lastComune] ['comune'] != $this->graph->edges [$edgeId]->nodeA) {
					...
					// se il pulman non ï¿½ stato gia' usato per questo comune
...
public function getTratte($percorso, &$trattaPartenza, &$trattaArrivo){
	$sql = "SELECT f.TrattaId FROM RT_Fermata f
			...
			WHERE f.ComuneId = ".$this->partenza. " AND t.LineaId = $this->idLinea AND t.Cancella = 0 AND t.Stato = 1 ORDER BY t.TrattaId ASC";
	$tratta = $this->conn->query_first ( $sql );	
	$trattaPartenza = $tratta['TrattaId'];

	$sql = "SELECT f.TrattaId FROM RT_Fermata f
			LEFT JOIN RT_Tratta t on t.TrattaId = f.TrattaId
			WHERE f.ComuneId = ".$this->destinazione. " AND t.LineaId = $this->idLinea AND t.Cancella = 0 AND t.Stato = 1 ORDER BY t.TrattaId ASC";
	$tratta = $this->conn->query_first ( $sql );
	$trattaArrivo = $tratta['TrattaId'];
	
	
	$resoconto = array();
	foreach($this->flotta as $k => $bus){
		for ($ii = 0; $ii < count($bus->comuni); $ii++){
			if(isset($bus->comuni[$ii]['passeggeri']) && count($bus->comuni[$ii]['passeggeri']) > 0 ){
				$sql = "SELECT f.TrattaId FROM RT_Fermata f
						LEFT JOIN RT_Tratta t on t.TrattaId = f.TrattaId
						WHERE f.ComuneId = '".$bus->comuni[$ii]['comune']. "' AND t.LineaId = $this->idLinea AND t.Cancella = 0 AND t.Stato = 1 ORDER BY t.TrattaId ASC";
				$tratta = $this->conn->query_first ( $sql );
				if(isset($bus->comuni[$ii+1]['comune'])) {
					if(isset($resoconto[$tratta['TrattaId']])){
						$resoconto[$tratta['TrattaId']] += $this->graph->edges[$bus->comuni[$ii]['comune']."-".$bus->comuni[$ii+1]['comune']]->peso;
					}else{
						$resoconto[$tratta['TrattaId']] = $this->graph->edges[$bus->comuni[$ii]['comune']."-".$bus->comuni[$ii+1]['comune']]->peso;
					}
				}
...
// 		echo "<br><br>corsaaa ".$this->idCorsa;
		if(isset($pickup) && isset($dropoff)){
			...
			$sql = "SELECT f.TrattaId FROM RT_Fermata f
					...
					f.IsBlackList = 0 AND
					f.WebSelling = 1 AND (
					f.ComuneId IN (SELECT FermataPickup FROM RT_CorsaTariffa WHERE CorsaId = ".$this->idCorsa.") OR
					f.ComuneId IN (SELECT FermataDropOff FROM RT_CorsaTariffa WHERE CorsaId = ".$this->idCorsa.")) AND
