...
class DisponibilitaGraph{
	...
	function __construct($idLinea, $idCorsa, $corsaDataPartenza, $conn, $postiMax, $crone = true){
...
// 			foreach ($this->graph->nodes as $key2=>$node2){
// 				if ($key != $key2 && !array_key_exists($key2, $this->graph->nodes[$key]->children)){
// 					$this->graph->connectNodes($key, $key2, 0, 10000);
// 					$this->graph->nodes[$key]->children[$key2] = $key2;
// 				}
				
// 				if ($key != $key2){
// 					$this->graph->nodes[$key]->descents[$key2] = $key2;
// 				}
...
// 				echo "<br>figlio $k";
	...
	// 				$sql = "select Posti from RT_DisponibilitaPostiCron where CorsaId = $this->idCorsa and DataPartenza = '$this->dataPartenza' and Comune = $k";
	// 				$row = $this->conn->query_first($sql);
	// 				if($row['Posti'] > 0){
...
// 			$this->gruppiDispo[$gruppoAvanza->posizione]->totalePasseggeri = $this->gruppiDispo[$gruppoAvanza->posizione]->totalePasseggeri + $this->graph->nodes[$gruppoAvanza->posizione]->salite;
			//discesa 
			...
			//salita
			if(isset($this->graph->nodes[$gruppoAvanza->posizione]->destinazioni)) {
