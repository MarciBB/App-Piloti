...
class Operatore {
	...
	function login($user_post, $pw_post, $sede_post) {
		...
		// $pass_crypt=$pw_post;
		$sql = "SELECT * From OperatoreLoginView WHERE Password='" . $pass_crypt . "' and Username='" . $db->escape ( $user_post ) . "' and RecuperaPwd<>1 and CodiceSede='$sede_post'";
		$row = $db->query_first ( $sql );
	...
	public function ControllModuloFunzionePermesso($ModuloId, $ModuloFunzioneId) {
		...
		$sql = "SELECT * From OperatorePermessi WHERE OperatoreId=$this->OperatoreId 
                ...
                and AppModuloFunzioneId=$ModuloFunzioneId and AppPermessoId<>4 and AppPermessoId<>4";
		return $db->fetch_array ( $sql );
	}
	...
	public function registrazione($gestoreId, $username, $password, $cognome,
			$nome, $email) {
		global $user;
		$db=$this->conn;
	
		$sql = "SELECT * FROM Operatore WHERE Username = '$username' OR Email = '$email'";
		...
		$data['Password'] = crypt($password,Config::$auth_salt);
		$data['GestoreId'] = $gestoreId;
		$data['Cognome'] = $cognome;
		$data['Nome'] = $nome;
		$data['Email'] = $email;
		$data['OperatoreTipoId'] = 1;
		$data['IsAdmin'] = 0;
		...
		$data['OpeIns'] = 13;
		$data['SedeIns'] = 1;
		$result = $db->insert("Operatore", $data);
		
		$dataRuolo['OperatoreId'] = $result;
		$dataRuolo['RuoloId'] = 306;
		$dataRuolo=$storico->operazioni_insert($dataRuolo,$user);
// 		$dataRuolo['OdcIdRef'] = 1;
// 		$dataRuolo['GestoreIdRef'] = 1;
// 		$dataRuolo['OpeIns'] = 13;
// 		$dataRuolo['SedeIns'] = 1;
		$result2 = $db->insert("OperatoreRuolo", $dataRuolo);
