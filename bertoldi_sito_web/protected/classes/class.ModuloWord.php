	...
	return $testo;
     ...
     function inizializza($codmodulo, $odc) {
         ...
         //   $sql = "SELECT * From WordViewModuloSegnalibro WHERE AppModuloWordId=$idModulo";
            ...
            while($row=$db->fetch($queryid)){
                ...
                if($view!=""){
						 ...
						 $conta=1;
                          while($row_1=$db->fetch($queryid_1)){
                           ...
                           $valore.=" ".$row_1['CognomeRagioneSociale'];
                            ...
                            $valore.=$res." ".$row_1['Indirizzo']." - ".$row_1['Cap']." - ".$row_1['Comune'];  
                          
                      if (mysql_num_rows($queryid_1)>$conta)
                            $valore.="; ";
                            $conta++;
					...
					elseif($campo=="ElencoParteIstanteDomicilioEletto"){
                         $sql_1="SELECT * FROM $view WHERE MediazioneId=$idMediazione and MediazioneSoggettoParteTipoId=1";
                         $queryid_1 = $db->query($sql_1);
                         $valore="";
						 $conta=1;
                          while($row_1=$db->fetch($queryid_1)){
                            ...
                            $indirizzo_completo="";
							if (!empty($row_1['IndirizzoDomicilio']))
							{
							$valore.=" ".$row_1['Destinatario'];
                            
                            $valore.="\\\line";
							$indirizzo=$row_1['IndirizzoDomicilio'];
							$cap=$row_1['CapDomicilio'];
							$comune=$row_1['ComuneDomicilio'];
							$indirizzo_completo=$indirizzo." \\\line ".$comune." - ".$cap;
							}
							else
							{
							$valore.=" ".$row_1['CognomeRagioneSociale'];
					...
					elseif($campo=="ElencoControparteDomicilioEletto"){
                         $sql_1="SELECT * FROM $view WHERE MediazioneId=$idMediazione and MediazioneSoggettoParteTipoId=2";
                         $queryid_1 = $db->query($sql_1);
                         $valore="";
						 $conta=1;
                     ...
                     elseif($bookmark=="##ParteIstanteElencoBase##"){
                         ...
                         $conta=1;
						  ...
						  elseif($bookmark=="##ParteIstanteConAvvocato##"){
                         $sql_1="SELECT * FROM $view WHERE MediazioneId=$idMediazione and MediazioneSoggettoParteTipoId=1";
                         ...
                         $conta=1;
                          while($row_1=$db->fetch($queryid_1)){
							...
							if(($row_1['AvvocatoCognome'])) 
							{	
							$valore.=", assistita dall'Avv. "	;
							if(($row_1['AvvocatoCognome'])) 
							$valore.=" ".$row_1['AvvocatoCognome'];


							if(($row_1['AvvocatoNome'])) 
							$valore.=" ".$row_1['AvvocatoNome'];
							}
                          ...
                          $content=str_replace($bookmark,utf8_decode($valore),$content);
						   ...
						   elseif($bookmark=="##ControParteIstanteConAvvocato##"){
                         $sql_1="SELECT * FROM $view WHERE MediazioneId=$idMediazione and MediazioneSoggettoParteTipoId=2";
                         $queryid_1 = $db->query($sql_1);
                         $valore="";
                         $conta=1;
        ...
        foreach($vars as $key=>$value) {
           ...
           // $key=str_replace("##","",$key);
            $search = $key;
			$value=$this->pulisci_lettura($value);
